  ...
  files?.forEach((f) => {
    ...
    f.functions?.forEach((fn) => {
      if (fn.complexity > 10)
        suggestions.push({
          level: "high",
          text: `Function '${fn.name}' in ${fname} has complexity ${fn.complexity} — consider breaking it into smaller, focused functions`,
        });
      if (fn.nestingDepth > 3)
        suggestions.push({
          level: "medium",
          text: `'${fn.name}' in ${fname} has ${fn.nestingDepth} nesting levels — extract inner blocks into helper functions`,
        });
      if (fn.paramCount > 4)
        suggestions.push({
          level: "medium",
          text: `'${fn.name}' takes ${fn.paramCount} parameters — consider grouping into a struct or config object`,
        });
    });
    if (f.totalLines > 250)
      suggestions.push({
        level: "medium",
        text: `${fname} is ${f.totalLines} lines — split into smaller modules for better maintainability`,
      });
    if (f.commentLines === 0 && f.codeLines > 50)
      suggestions.push({
        level: "low",
        text: `${fname} has no comments — add documentation for public functions`,
      });
  });
  if (score < 70)
    suggestions.unshift({
      level: "high",
      text: `Overall health score is ${score.toFixed(1)} — prioritize refactoring high-complexity files first`,
    });
